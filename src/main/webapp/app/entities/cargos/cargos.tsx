...
export interface ICargosState {
  search: string;
}

export class Cargos extends React.Component<ICargosProps, ICargosState> {
  state: ICargosState = {
    search: ''
  };
  ...
  render() {
    const { cargosList, match } = this.props;
    return (
      <div>
        <h2 id="cargos-heading">
          ...
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            ...
            &nbsp;
            <Translate contentKey="agamottoApp.cargos.home.createLabel">Create a new Cargos</Translate>
          </Link>
        </h2>
        <Row>
          <Col sm="12">
        ...
        <div className="table-responsive">
          {cargosList && cargosList.length > 0 ? (
            <Table responsive aria-describedby="cargos-heading">
              <thead>
                <tr>
                  <th>
              ...
              <tbody>
                {cargosList.map((cargos, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{cargos.nome}</td>
                    <td>
                      {cargos.colaborador ? <Link to={`colaboradores/${cargos.colaborador.id}`}>{cargos.colaborador.nome}</Link> : ''}
                    </td>
