...
export interface IControlePontoUpdateState {
  isNew: boolean;
  colaboradorId: string;
}

export class ControlePontoUpdate extends React.Component<IControlePontoUpdateProps, IControlePontoUpdateState> {
  constructor(props) {
    super(props);
    this.state = {
      colaboradorId: '0',
  ...
  handleClose = () => {
    this.props.history.push('/controle-ponto');
  };

  render() {
    const { controlePontoEntity, colaboradores, loading, updating } = this.props;
        ...
        <Row className="justify-content-center">
          <Col md="8">
            {loading ? (
              ...
              <AvForm model={isNew ? {} : controlePontoEntity} onSubmit={this.saveEntity}>
                {!isNew ? (
                  <AvGroup>
